...
import { domain, getFile } from './utils/unpkg'
...
function detectModeFromFileExtension(fileType: string) {
  if (/\.m?js/.test(fileType)) {
    return 'javascript'
  } else if (/\.jsx/.test(fileType)) {
    return 'jsx'
...
const encodeSelection = ([from, to]) =>
  `${from.row + 1}:${from.column}-${to.row + 1}:${to.column}`
...
const decodeSelection = (string) => {
  ...
  return [
    { row: (+fromRow || 1) - 1, column: +fromColumn || 0 },
    { row: (+toRow || 1) - 1, column: +toColumn || 0 }
  ]
}
...
export default class FileBrowser extends React.Component {
  ...
  onLoad = (aceEditor) => {
    ...
    this.updateSize()
    const [selectionFrom, selectionTo] = decodeSelection(
      getQueryString().selection || ''
    )
    aceEditor.focus()
    aceEditor.gotoLine(selectionFrom.row, selectionFrom.column, true)
    aceEditor.selection.setSelectionRange(
      { start: selectionFrom, end: selectionTo },
      false
    )
  ...
  requestFile(file) {
    if (!file || !this.props.package) return null
