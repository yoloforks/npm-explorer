...
import { getReadableSize } from './utils/etc'
import { openTreeToFile } from './utils/fileOps'
import { getPackage } from './utils/unpkg'
import { getQueryString, replaceUrlState } from './utils/url'

const getAuthorElement = (packageAuthor) => {
  if (!packageAuthor) return ''

  if (typeof packageAuthor === 'string') return packageAuthor
  ...
  if (packageAuthor instanceof Array) return 'multiple authors'
...
export default class App extends React.Component {
  ...
  async componentDidMount() {
    ...
    const selectedFile =
      path.length > 0 ? openTreeToFile(packageInfo.files || [], path) : null
  ...
  render() {
    ...
    return (
      <div
        className={`layout layout-${layout} ${
          this.state.selectedFile ? 'file-selected' : ''
        }`}
      >
        <div className="header">
          <a
            href="https://github.com/ZitRos/npm-explorer/blob/master/readme.md"
            target="_blank"
          ...
          <div className="top">
            <span className="name">
              ...
              @{this.state.data.package.version || '...'}
            </span>
            &nbsp;
        ...
        <div className="col2">
          <div className="head">
            ...
            {this.state.selectedFile ? (
              <div className="file-name">{this.state.selectedFile.path}</div>
            ) : null}
          </div>
