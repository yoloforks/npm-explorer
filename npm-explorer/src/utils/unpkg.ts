...
export async function getPackage(packageName) {
  ...
  try {
    meta = await new Promise((yes, no) =>
      get(filesUrl, (err, data, postUrl) => {
        if (err) return no(err)
        if (data && data.files && data.files.length > 0) return yes(data)
  ...
  try {
    pkg = await new Promise((yes, no) =>
      get(packageUrl, (err, data) => (err ? no(err) : yes(data)))
    )
  } catch (e) {
    return {
      error: e
 ...
 * @return {Promise.<void>}
 */
export async function getFile(packageName, filePath) {
